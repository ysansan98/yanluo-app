...
<template>
  ...
  >
    <header class="mx-auto mb-6 max-w-225">
      <div class="text-3xl font-bold tracking-[0.5px]">Yanluo ASR</div>
    ...
    >
      <div class="mb-3 text-base font-bold text-[#1637b8]">Live Dictation</div>
      <div
      ...
      >
        <label class="w-40 font-semibold text-[#2f2f2f] max-[720px]:w-auto"
          >Continue window (ms)</label
        >
        <input
      ...
      <div class="mt-1.5 min-h-4.5 text-[#555]">Status: {{ liveStatus }}</div>
      <div class="mt-1.5 min-h-4.5 text-[#555]">
        Live elapsed: {{ liveElapsedMs }} ms
      </div>
      <div class="mb-2 mt-3 font-semibold text-[#2f2f2f]">Partial</div>
      <div
      ...
      <div class="mb-2 mt-3 font-semibold text-[#2f2f2f]">Final</div>
      <div
        class="min-h-14 w-full wrap-break-word rounded-xl border border-[#d5d5db] bg-white p-3 text-[13px] leading-6 whitespace-pre-wrap text-[#111] font-['IBM_Plex_Mono',monospace]"
      >
        {{ liveFinalText || "-" }}
